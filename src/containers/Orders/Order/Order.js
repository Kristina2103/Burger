...
const Order = (props) => {
    const customerDetails =Object.keys(props.customerDetails).map(key => {
        return <li className={classes.CustomerDetails} key={key} >
            <span>{key}</span> : {props.customerDetails[key]} </li>
      })
      
    const ingredients = props.orderDetails.map((order, index)=>{
       return <li className={classes.Ingredient} key={index} >
           <span>{order.name}</span> : {order.qty} x {order.price}RSD</li>
    })
